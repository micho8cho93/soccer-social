...
from .models import Player, PlayerStatistic, Match, TournamentMatch, TournamentPlayerStatistic
...
class TournamentRosterUpdateForm(forms.Form):
    def __init__(self, *args, **kwargs):
        self.tournament_match = kwargs.pop('tournament_match')
        self.user = kwargs.pop('user')
        super().__init__(*args, **kwargs)

        home_players_queryset = Player.objects.filter(team=self.tournament_match.home_team)
        away_players_queryset = Player.objects.filter(team=self.tournament_match.away_team)

        self.fields['home_team_players'] = forms.ModelMultipleChoiceField(
            queryset=home_players_queryset,
            widget=forms.CheckboxSelectMultiple,
            required=False,
            label=f'{self.tournament_match.home_team.name} Players'
        )
        self.fields['away_team_players'] = forms.ModelMultipleChoiceField(
            queryset=away_players_queryset,
            widget=forms.CheckboxSelectMultiple,
            required=False,
            label=f'{self.tournament_match.away_team.name} Players'
        )

        # Set initial values based on existing TournamentPlayerStatistic.present
        initial_home_players = [ps.player for ps in TournamentPlayerStatistic.objects.filter(tournament_match=self.tournament_match, player__in=home_players_queryset, present=True)]
        initial_away_players = [ps.player for ps in TournamentPlayerStatistic.objects.filter(tournament_match=self.tournament_match, player__in=away_players_queryset, present=True)]
        self.initial['home_team_players'] = initial_home_players
        self.initial['away_team_players'] = initial_away_players
    ...
    def update_roster_data(self):
        ...
        # Only update players for the user's associated team, or if superuser
        if self.user.is_superuser or user_team == self.tournament_match.home_team:
            for player in Player.objects.filter(team=self.tournament_match.home_team):
                player_stat, created = TournamentPlayerStatistic.objects.get_or_create(player=player, tournament_match=self.tournament_match)
                player_stat.present = player in present_home_players
                player_stat.save()

        if self.user.is_superuser or user_team == self.tournament_match.away_team:
            for player in Player.objects.filter(team=self.tournament_match.away_team):
                player_stat, created = TournamentPlayerStatistic.objects.get_or_create(player=player, tournament_match=self.tournament_match)
                player_stat.present = player in present_away_players
                player_stat.save()